...
struct directedGraph *DAGinput(char *file){
    ...
    if(topologicalSortBool(dag)==0){
        printf("This graph is not a Directed Acyclic Graph! Please check input file.");
        exit(255);
    }
...
void lcaDAGInput(struct directedGraph *g){
    ...
    while(exit != 3){
        ...
        if(scanf("%d%c", &num, &term) != 2 || term != '\n'){
            ...
            adjnode *temp = g->adjListArr[num].head;
            if(temp == NULL){
                printf("Sorry that key is not in the BT , please try again !\n");
            }else{
                if(exit == 1) k1 = num;
